/* Tiếng Việt — Vietnam. See ./types.ts: chemical names, grade names and division
   numbers stay in the original. */

import type { Dict } from "./types";

const vi: Dict = {
  // --- navigation and header ------------------------------------------------
  "Home": "Trang chủ",
  "About us": "Về chúng tôi",
  "Divisions": "Bộ phận",
  "Products": "Sản phẩm",
  "Industries": "Ngành công nghiệp",
  "Applications": "Ứng dụng",
  "Capabilities": "Năng lực",
  "R&D": "R&D",
  "Innovation": "Đổi mới",
  "Sustainability": "Phát triển bền vững",
  "Contact": "Liên hệ",
  "Product Finder": "Tra cứu sản phẩm",
  "Skip to content": "Bỏ qua tới nội dung",
  "Search the whole range": "Tìm trong toàn bộ danh mục",
  "Visit page": "Xem trang",
  "Sections": "Chuyên mục",
  "On the home page": "Trên trang chủ",
  "Explore": "Tìm hiểu",
  "Previous": "Trước",
  "Next": "Tiếp",
  "Previous picture": "Ảnh trước",
  "Next picture": "Ảnh tiếp theo",
  "Business units": "Đơn vị kinh doanh",
  "Business Units": "Đơn vị kinh doanh",
  "Research & Development": "Nghiên cứu & phát triển",

  // --- country and language menu -------------------------------------------
  "Region & language": "Khu vực & ngôn ngữ",
  "Change country and language": "Đổi quốc gia và ngôn ngữ",
  "Choose where you are buying from. We manufacture in Surat and export worldwide — the site switches language with the country.":
    "Hãy chọn quốc gia bạn mua hàng. Chúng tôi sản xuất tại Surat và xuất khẩu toàn cầu — ngôn ngữ trang web thay đổi theo quốc gia.",
  "Navigation and product titles are translated. Detailed page copy remains in English.":
    "Phần điều hướng và tên sản phẩm đã được dịch. Nội dung chi tiết của trang vẫn giữ nguyên tiếng Anh.",
  "Detected": "Tự động",
  "Search countries": "Tìm quốc gia",
  "Search {n} countries": "Tìm trong {n} quốc gia",
  "Set from your device's time zone. Change it here at any time.":
    "Được chọn theo múi giờ trên thiết bị của bạn. Bạn có thể thay đổi tại đây bất cứ lúc nào.",
  "Set from your browser's language. Change it here at any time.":
    "Được chọn theo ngôn ngữ trình duyệt của bạn. Bạn có thể thay đổi tại đây bất cứ lúc nào.",
  "No country matches “{q}”. We export worldwide — write to us and we will route your enquiry.":
    "Không có quốc gia nào khớp với “{q}”. Chúng tôi xuất khẩu toàn cầu — hãy liên hệ và chúng tôi sẽ chuyển yêu cầu của bạn đến đúng bộ phận.",

  // the six regions the panel groups the countries into
  "Asia Pacific": "Châu Á – Thái Bình Dương",
  "Europe": "Châu Âu",
  "Americas": "Châu Mỹ",
  "Middle East": "Trung Đông",
  "Africa": "Châu Phi",
  "Central Asia & Caucasus": "Trung Á & Kavkaz",

  /* All seventy countries in `lib/i18n/locales.ts`, in the order the panel
     lists them. A country whose name is the same in this language is still
     written out, so a name added to the register shows up here as a gap
     rather than silently reading as translated. */
  // Asia Pacific
  "India": "Ấn Độ",
  "China": "Trung Quốc",
  "Japan": "Nhật Bản",
  "South Korea": "Hàn Quốc",
  "Taiwan": "Đài Loan",
  "Vietnam": "Việt Nam",
  "Indonesia": "Indonesia",
  "Malaysia": "Malaysia",
  "Singapore": "Singapore",
  "Thailand": "Thái Lan",
  "Philippines": "Philippines",
  "Pakistan": "Pakistan",
  "Bangladesh": "Bangladesh",
  "Nepal": "Nepal",
  "Sri Lanka": "Sri Lanka",
  "Myanmar": "Myanmar",
  "Cambodia": "Campuchia",
  "Australia": "Úc",
  "New Zealand": "New Zealand",
  // Europe
  "United Kingdom": "Vương quốc Anh",
  "Ireland": "Ireland",
  "Germany": "Đức",
  "France": "Pháp",
  "Italy": "Ý",
  "Spain": "Tây Ban Nha",
  "Portugal": "Bồ Đào Nha",
  "Netherlands": "Hà Lan",
  "Belgium": "Bỉ",
  "Switzerland": "Thụy Sĩ",
  "Austria": "Áo",
  "Sweden": "Thụy Điển",
  "Norway": "Na Uy",
  "Denmark": "Đan Mạch",
  "Finland": "Phần Lan",
  "Poland": "Ba Lan",
  "Czech Republic": "Cộng hòa Séc",
  "Hungary": "Hungary",
  "Romania": "Romania",
  "Greece": "Hy Lạp",
  "Russia": "Nga",
  "Ukraine": "Ukraina",
  "Turkey": "Thổ Nhĩ Kỳ",
  // Americas
  "United States": "Hoa Kỳ",
  "Canada": "Canada",
  "Mexico": "Mexico",
  "Brazil": "Brazil",
  "Argentina": "Argentina",
  "Chile": "Chile",
  "Colombia": "Colombia",
  "Peru": "Peru",
  // Middle East
  "United Arab Emirates": "Các Tiểu vương quốc Ả Rập Thống nhất",
  "Saudi Arabia": "Ả Rập Xê Út",
  "Qatar": "Qatar",
  "Kuwait": "Kuwait",
  "Oman": "Oman",
  "Bahrain": "Bahrain",
  "Israel": "Israel",
  // Africa
  "Egypt": "Ai Cập",
  "Morocco": "Maroc",
  "Algeria": "Algeria",
  "Nigeria": "Nigeria",
  "Ghana": "Ghana",
  "Kenya": "Kenya",
  "Tanzania": "Tanzania",
  "Ethiopia": "Ethiopia",
  "South Africa": "Nam Phi",
  // Central Asia & Caucasus
  "Kazakhstan": "Kazakhstan",
  "Uzbekistan": "Uzbekistan",
  "Azerbaijan": "Azerbaijan",
  "Georgia": "Gruzia",

  // --- nav dropdown introductions -------------------------------------------
  "Speciality chemistry manufactured and exported from Surat — ten divisions, 112 grades, one integrated plant.":
    "Hóa chất đặc chủng được sản xuất và xuất khẩu từ Surat — mười bộ phận, 112 mác sản phẩm, một nhà máy tích hợp.",
  "Who we are: the mandate to manufacture and export chemistry that makes industry safer, cleaner and more efficient.":
    "Chúng tôi là ai: sứ mệnh sản xuất và xuất khẩu hóa chất giúp ngành công nghiệp an toàn hơn, sạch hơn và hiệu quả hơn.",
  "Ten specialised divisions under one roof, structured so scale never costs you technical attention.":
    "Mười bộ phận chuyên môn dưới một mái nhà, được tổ chức sao cho quy mô không bao giờ làm giảm sự hỗ trợ kỹ thuật.",
  "The full range — 112 grades built for high performance, consistency and regulatory confidence.":
    "Toàn bộ danh mục — 112 mác sản phẩm cho hiệu năng cao, độ ổn định và sự yên tâm về pháp lý.",
  "The sectors we formulate for, and what each one asks of a speciality chemical supplier.":
    "Những ngành mà chúng tôi phát triển công thức, và điều mỗi ngành đòi hỏi ở một nhà cung cấp hóa chất đặc chủng.",
  "What our chemistry actually does in service — the end uses each grade was developed for.":
    "Hóa chất của chúng tôi thực sự làm gì khi vận hành — những ứng dụng cuối mà mỗi mác sản phẩm được phát triển cho.",
  "Manufacturing, laboratory, quality and export capability, described plainly.":
    "Năng lực sản xuất, phòng thí nghiệm, chất lượng và xuất khẩu, được trình bày rõ ràng.",
  "The laboratories, instruments and synthesis work behind every grade we ship.":
    "Phòng thí nghiệm, thiết bị và công việc tổng hợp đằng sau mỗi mác sản phẩm chúng tôi giao.",
  "Where the next products come from — in the flask, on the plant floor, and with customers.":
    "Sản phẩm tiếp theo ra đời từ đâu — trong bình phản ứng, trên sàn nhà máy và cùng với khách hàng.",
  "How the plant, the portfolio and the people are being held to a lower footprint.":
    "Cách nhà máy, danh mục sản phẩm và con người được hướng tới dấu chân môi trường thấp hơn.",

  // --- on-page jump links ---------------------------------------------------
  "What we make": "Chúng tôi sản xuất gì",
  "Our products": "Sản phẩm của chúng tôi",
  "Find a product": "Tìm sản phẩm",
  "Ten divisions": "Mười bộ phận",
  "Industries served": "Ngành phục vụ",
  "A force for good": "Một lực lượng vì điều tốt đẹp",
  "Latest media": "Tin mới nhất",
  "Global presence": "Hiện diện toàn cầu",
  "Manufacturing excellence": "Năng lực sản xuất",
  "Research & development": "Nghiên cứu & phát triển",
  "Quality & compliance": "Chất lượng & tuân thủ",
  "Our vision": "Tầm nhìn",

  // --- topic titles ---------------------------------------------------------
  "Lubrication & Grease": "Bôi trơn & mỡ bôi trơn",
  "Flame Retardancy": "Chống cháy",
  "Polymers & Engineering": "Polyme & kỹ thuật",
  "Electrical & Electronics": "Điện & điện tử",
  "Automotive": "Ô tô",
  "Industrial Machinery & Equipment": "Máy móc & thiết bị công nghiệp",
  "Coatings & Composites": "Lớp phủ & vật liệu composite",
  "Pharmaceutical & Agrochemical": "Dược phẩm & hóa chất nông nghiệp",
  "Energy Storage & Electronics": "Lưu trữ năng lượng & điện tử",
  "Nutraceutical, Cosmetic & Food": "Thực phẩm chức năng, mỹ phẩm & thực phẩm",
  "Automotive & Transportation": "Ô tô & vận tải",
  "Polymers & Engineering Plastics": "Polyme & nhựa kỹ thuật",
  "Construction & Infrastructure": "Xây dựng & hạ tầng",
  "Research-Driven Specialty Industries": "Ngành đặc chủng dẫn dắt bởi nghiên cứu",
  "Lithium & Metal Additives": "Phụ gia lithium & kim loại",
  "Inorganic & Flame-Retardant Chemicals": "Hóa chất vô cơ & chống cháy",
  "Organic & Pharmaceutical Intermediates": "Trung gian hữu cơ & dược phẩm",
  "Polymers, Resins & High-Performance Materials": "Polyme, nhựa & vật liệu hiệu năng cao",
  "Surfactants, Detergents & Bio-Based Chemicals": "Chất hoạt động bề mặt, chất tẩy rửa & hóa chất sinh học",
  "Electrochemical, Battery & Electronic Chemicals": "Hóa chất điện hóa, pin & điện tử",
  "Paints, Coatings & Pigments": "Sơn, lớp phủ & bột màu",
  "Nutraceutical, Cosmetic & Food Chemicals": "Hóa chất thực phẩm chức năng, mỹ phẩm & thực phẩm",
  "Water Treatment & Industrial Additives": "Xử lý nước & phụ gia công nghiệp",
  "Functional & High-Performance Materials": "Vật liệu chức năng & hiệu năng cao",
  "Flame-Retardant & Inorganic Salts": "Muối chống cháy & vô cơ",
  "Advanced Manufacturing": "Sản xuất tiên tiến",
  "Research & Development Excellence": "Xuất sắc trong nghiên cứu & phát triển",
  "Quality Assurance & Compliance": "Đảm bảo chất lượng & tuân thủ",
  "Technical & Application Support": "Hỗ trợ kỹ thuật & ứng dụng",
  "Global Export & Logistics": "Xuất khẩu toàn cầu & logistics",
  "Sustainability Integration": "Tích hợp phát triển bền vững",
  "Product Innovation": "Đổi mới sản phẩm",
  "Process & Manufacturing Innovation": "Đổi mới quy trình & sản xuất",
  "Application & Industry-Specific Innovation": "Đổi mới theo ứng dụng & ngành",
  "Sustainable & Green Innovation": "Đổi mới bền vững & xanh",
  "Collaboration & Knowledge Innovation": "Đổi mới hợp tác & tri thức",
  "Integrated Innovation Advantage": "Lợi thế đổi mới tích hợp",
  "Product Innovation & Development": "Đổi mới & phát triển sản phẩm",
  "Advanced Analytical & Testing": "Phân tích & thử nghiệm tiên tiến",
  "Application-Specific Research": "Nghiên cứu theo ứng dụng",
  "Sustainable & Green Chemistry": "Hóa học bền vững & xanh",
  "Collaboration & Knowledge Sharing": "Hợp tác & chia sẻ tri thức",
  "Integrated R&D Excellence": "Xuất sắc R&D tích hợp",
  "Environmental Stewardship": "Trách nhiệm môi trường",
  "Sustainable Product Innovation": "Đổi mới sản phẩm bền vững",
  "Energy & Resource Optimisation": "Tối ưu năng lượng & tài nguyên",
  "Health, Safety & Community": "Sức khỏe, an toàn & cộng đồng",
  "Circular Economy & Waste": "Kinh tế tuần hoàn & chất thải",
  "Integrated Sustainability Excellence": "Xuất sắc bền vững tích hợp",

  // --- topic blurbs: applications -------------------------------------------
  "Lithium Myristate · 12-Hydroxy Lithium Stearate · Calcium Stearate — for automotive, heavy machinery, industrial equipment, polymers & plastics":
    "Lithium Myristate · 12-Hydroxy Lithium Stearate · Calcium Stearate — cho ô tô, máy móc hạng nặng, thiết bị công nghiệp, polyme & nhựa",
  "Aluminium · Calcium · Magnesium Hypophosphite — for plastics, electronics, electrical, coatings and construction materials":
    "Aluminium · Calcium · Magnesium Hypophosphite — cho nhựa, điện tử, điện, lớp phủ và vật liệu xây dựng",
  "Lithium soaps · Hypophosphite salts — for plastics, engineering materials, automotive and electronics":
    "Xà phòng lithium · Muối hypophosphite — cho nhựa, vật liệu kỹ thuật, ô tô và điện tử",
  "Aluminium Hypophosphite · Magnesium Hypophosphite — for consumer electronics, electrical appliances and industrial electronics":
    "Aluminium Hypophosphite · Magnesium Hypophosphite — cho điện tử tiêu dùng, thiết bị điện và điện tử công nghiệp",
  "Lithium soaps & greases · Hypophosphite salts · Polymer additives — for automotive components, commercial vehicles and heavy machinery":
    "Xà phòng & mỡ lithium · Muối hypophosphite · Phụ gia polyme — cho linh kiện ô tô, xe thương mại và máy móc hạng nặng",
  "Lithium soaps · Organic & pharmaceutical intermediates — for heavy machinery, manufacturing plants and industrial equipment":
    "Xà phòng lithium · Trung gian hữu cơ & dược phẩm — cho máy móc hạng nặng, nhà máy sản xuất và thiết bị công nghiệp",
  "Hypophosphite salts · Organic & pharmaceutical intermediates — for construction, industrial coatings and composites manufacturing":
    "Muối hypophosphite · Trung gian hữu cơ & dược phẩm — cho xây dựng, lớp phủ công nghiệp và sản xuất composite",
  "1-Phenyl Oxide · Triazolo Pyridione — for pharmaceutical, agrochemical and industrial R&D":
    "1-Phenyl Oxide · Triazolo Pyridione — cho R&D dược phẩm, hóa chất nông nghiệp và công nghiệp",
  "Lithium Carbonate · Electrolyte salts · TMAH · Electronic-grade solvents — for batteries, energy storage, semiconductors and electronics":
    "Lithium Carbonate · Muối điện giải · TMAH · Dung môi cấp điện tử — cho pin, lưu trữ năng lượng, bán dẫn và điện tử",
  "Ascorbic Acid · Niacinamide · Hyaluronic Acid · PABA — for nutraceuticals, cosmetics, functional foods and pharmaceuticals":
    "Ascorbic Acid · Niacinamide · Hyaluronic Acid · PABA — cho thực phẩm chức năng, mỹ phẩm, thực phẩm chức năng và dược phẩm",

  // --- topic blurbs: industries ---------------------------------------------
  "Lithium Myristate · 12-Hydroxy Lithium Stearate · Hypophosphite salts · Polymer additives":
    "Lithium Myristate · 12-Hydroxy Lithium Stearate · Muối hypophosphite · Phụ gia polyme",
  "Lithium soaps · Aluminium Hypophosphite · Magnesium Hypophosphite":
    "Xà phòng lithium · Aluminium Hypophosphite · Magnesium Hypophosphite",
  "Aluminium Hypophosphite · Magnesium Hypophosphite":
    "Aluminium Hypophosphite · Magnesium Hypophosphite",
  "Lithium soaps · Organic & pharmaceutical intermediates":
    "Xà phòng lithium · Trung gian hữu cơ & dược phẩm",
  "Hypophosphite salts · Organic & pharmaceutical intermediates":
    "Muối hypophosphite · Trung gian hữu cơ & dược phẩm",
  "1-Phenyl Oxide · Triazolo Pyridione": "1-Phenyl Oxide · Triazolo Pyridione",
  "Lithium Carbonate · Electrolyte salts · TMAH · Electronic-grade solvents":
    "Lithium Carbonate · Muối điện giải · TMAH · Dung môi cấp điện tử",
  "Ascorbic Acid · Niacinamide · Hyaluronic Acid · PABA":
    "Ascorbic Acid · Niacinamide · Hyaluronic Acid · PABA",
  "Graphene & nano additives · High-performance polymers · Functional coatings":
    "Phụ gia graphene & nano · Polyme hiệu năng cao · Lớp phủ chức năng",

  // --- topic blurbs: business units -----------------------------------------
  "Manufacturing and export of lithium-based metal soaps and metal additives for industrial lubrication systems.":
    "Sản xuất và xuất khẩu xà phòng kim loại gốc lithium và phụ gia kim loại cho hệ thống bôi trơn công nghiệp.",
  "Production and export of halogen-free inorganic flame-retardant salts and functional metal salts.":
    "Sản xuất và xuất khẩu muối chống cháy vô cơ không chứa halogen và muối kim loại chức năng.",
  "Manufacture and export of high-purity organic intermediates for pharmaceutical, agrochemical and industrial chemical applications.":
    "Sản xuất và xuất khẩu các chất trung gian hữu cơ độ tinh khiết cao cho ứng dụng dược phẩm, hóa chất nông nghiệp và hóa chất công nghiệp.",
  "Advanced polymers, resins, adhesives, sealants and functional materials for industrial applications.":
    "Polyme, nhựa, keo dán, chất trám và vật liệu chức năng tiên tiến cho ứng dụng công nghiệp.",
  "Industrial surfactants, emulsifiers, detergents and bio-based chemical solutions.":
    "Chất hoạt động bề mặt công nghiệp, chất nhũ hóa, chất tẩy rửa và giải pháp hóa chất sinh học.",
  "Chemicals for lithium-ion batteries, electronics and high-purity semiconductor applications.":
    "Hóa chất cho pin lithium-ion, điện tử và ứng dụng bán dẫn độ tinh khiết cao.",
  "High-value pigments, coatings intermediates and additive solutions.":
    "Bột màu giá trị cao, chất trung gian cho lớp phủ và giải pháp phụ gia.",
  "Vitamins, nutraceuticals, cosmetic actives and functional food additives.":
    "Vitamin, thực phẩm chức năng, hoạt chất mỹ phẩm và phụ gia thực phẩm chức năng.",
  "Chemicals for industrial water treatment, corrosion inhibition, anti-scaling and functional additives.":
    "Hóa chất xử lý nước công nghiệp, ức chế ăn mòn, chống cáu cặn và phụ gia chức năng.",
  "Niche chemicals for aerospace, automotive, electronics and industrial applications.":
    "Hóa chất chuyên biệt cho hàng không vũ trụ, ô tô, điện tử và ứng dụng công nghiệp.",

  // --- topic blurbs: product groups -----------------------------------------
  "High-performance thickeners for automotive and industrial greases, with excellent thermal stability, water resistance and mechanical durability.":
    "Chất làm đặc hiệu năng cao cho mỡ bôi trơn ô tô và công nghiệp, với độ ổn định nhiệt, khả năng kháng nước và độ bền cơ học vượt trội.",
  "Halogen-free additives that enhance fire resistance in plastics, cables and coatings while reducing smoke and toxicity.":
    "Phụ gia không chứa halogen giúp tăng khả năng chống cháy cho nhựa, cáp và lớp phủ, đồng thời giảm khói và độc tính.",
  "High-purity compounds for pharmaceuticals, agrochemicals and industrial applications — scalable from lab research to industrial production without compromising quality.":
    "Hợp chất độ tinh khiết cao cho dược phẩm, hóa chất nông nghiệp và công nghiệp — mở rộng quy mô từ nghiên cứu phòng thí nghiệm đến sản xuất công nghiệp mà không ảnh hưởng chất lượng.",
  "Advanced polymers and resins for adhesives, coatings and functional materials, including heat-resistant grades for aerospace and automotive use.":
    "Polyme và nhựa tiên tiến cho keo dán, lớp phủ và vật liệu chức năng, bao gồm các mác chịu nhiệt cho hàng không vũ trụ và ô tô.",
  "Industrial and green surfactants, emulsifiers and wetting agents, plus eco-friendly plant-based intermediates and solvents.":
    "Chất hoạt động bề mặt công nghiệp và thân thiện môi trường, chất nhũ hóa và chất thấm ướt, cùng các chất trung gian và dung môi gốc thực vật.",
  "Lithium-ion battery salts, high-purity solvents and semiconductor chemicals with ultra-high purity and application-specific performance.":
    "Muối pin lithium-ion, dung môi độ tinh khiết cao và hóa chất bán dẫn với độ tinh khiết siêu cao và hiệu năng theo từng ứng dụng.",
  "Organic and inorganic pigments, coating additives and UV stabilisers delivering consistent colour, dispersion and performance.":
    "Bột màu hữu cơ và vô cơ, phụ gia lớp phủ và chất ổn định UV mang lại màu sắc, độ phân tán và hiệu năng ổn định.",
  "High-purity intermediates for R&D and commercial applications, with custom formulations and application-specific support.":
    "Chất trung gian độ tinh khiết cao cho R&D và ứng dụng thương mại, với công thức riêng và hỗ trợ theo từng ứng dụng.",
  "Customised formulations for operational efficiency and environmental compliance in cooling towers, power plants and process industries.":
    "Công thức tùy chỉnh cho hiệu quả vận hành và tuân thủ môi trường tại tháp giải nhiệt, nhà máy điện và các ngành chế biến.",
  "R&D-driven solutions for next-generation material challenges across aerospace, automotive, electronics and advanced industry.":
    "Giải pháp dẫn dắt bởi R&D cho các thách thức vật liệu thế hệ mới trong hàng không vũ trụ, ô tô, điện tử và công nghiệp tiên tiến.",

  // --- topic and grade page chrome ------------------------------------------
  "The range": "Danh mục",
  "Grades in this group": "Các mác trong nhóm này",
  "All applications": "Tất cả ứng dụng",
  "All industries": "Tất cả ngành",
  "All business units": "Tất cả đơn vị kinh doanh",
  "All products": "Tất cả sản phẩm",
  "All capabilities": "Tất cả năng lực",
  "All innovation": "Tất cả đổi mới",
  "All r&d": "Tất cả R&D",
  "All sustainability": "Tất cả nội dung bền vững",
  "Talk to our technical team": "Trao đổi với đội ngũ kỹ thuật",
  "Technical data, packaging options and export documentation — tell us what you need and the right team will come back to you.":
    "Dữ liệu kỹ thuật, phương án đóng gói và chứng từ xuất khẩu — hãy cho chúng tôi biết nhu cầu của bạn và bộ phận phù hợp sẽ liên hệ lại.",
  "Start a conversation": "Bắt đầu trao đổi",
  "Division": "Bộ phận",
  "Product group": "Nhóm sản phẩm",
  "Product name": "Tên sản phẩm",
  "All product names": "Tất cả tên sản phẩm",
  "Filtered under": "Được phân loại theo",
  "Also in this group": "Sản phẩm khác trong nhóm",
  "Request technical data": "Yêu cầu dữ liệu kỹ thuật",
  "Technical and safety documentation, packaging options and export paperwork — tell us the specification you need to hit.":
    "Tài liệu kỹ thuật và an toàn, phương án đóng gói và chứng từ xuất khẩu — hãy cho chúng tôi biết thông số bạn cần đạt.",
  "Request TDS / SDS": "Yêu cầu TDS / SDS",
  "Back to the finder": "Quay lại tra cứu sản phẩm",

  // --- product finder -------------------------------------------------------
  "Filter by": "Lọc theo",
  "Clear filters": "Xóa bộ lọc",
  "Search…": "Tìm kiếm…",
  "Search products": "Tìm kiếm sản phẩm",
  "Clear search": "Xóa tìm kiếm",
  "Displaying {range} of {total} Results": "Hiển thị {range} trong {total} kết quả",
  "Displaying {total} Results": "Hiển thị {total} kết quả",
  "No products match those filters. Clear a filter, or {ask}.":
    "Không có sản phẩm nào khớp với bộ lọc. Hãy bỏ bớt một bộ lọc, hoặc {ask}.",
  "ask us about a custom grade": "hỏi chúng tôi về mác sản phẩm riêng",
  "Show more results": "Xem thêm kết quả",
  "All divisions": "Tất cả bộ phận",
  "Automotive & transportation": "Ô tô & vận tải",
  "Polymers & engineering plastics": "Polyme & nhựa kỹ thuật",
  "Electrical & electronics": "Điện & điện tử",
  "Industrial machinery": "Máy móc công nghiệp",
  "Coatings & composites": "Lớp phủ & composite",
  "Pharmaceutical & agrochemical": "Dược phẩm & hóa chất nông nghiệp",
  "Construction & infrastructure": "Xây dựng & hạ tầng",
  "Energy storage": "Lưu trữ năng lượng",
  "Nutraceutical, cosmetic & food": "Thực phẩm chức năng, mỹ phẩm & thực phẩm",
  "Water treatment": "Xử lý nước",
  "Lithium & metal additives": "Phụ gia lithium & kim loại",
  "Flame retardants": "Chất chống cháy",
  "Intermediates": "Chất trung gian",
  "Polymers & resins": "Polyme & nhựa",
  "Surfactants": "Chất hoạt động bề mặt",
  "Battery & electronic": "Pin & điện tử",
  "Pigments": "Bột màu",
  "Nutraceutical & cosmetic": "Thực phẩm chức năng & mỹ phẩm",
  "Functional materials": "Vật liệu chức năng",

  // --- banner headlines ------------------------------------------------------
  // The whole two-tone headline as one key; see components/HeroTitle.tsx for why
  // the light/bold split is not translated as two fragments.
  "About Cosmox Chemicals": "Về Cosmox Chemicals",
  "Contact Us": "Liên hệ với chúng tôi",
  "Search for a Product": "Tìm sản phẩm",
  "Industries We Serve": "Các ngành chúng tôi phục vụ",

  // --- footer ---------------------------------------------------------------
  "Advanced chemical manufacturing and export — enabling industrial efficiency, safety and sustainability worldwide.":
    "Sản xuất và xuất khẩu hóa chất tiên tiến — góp phần nâng cao hiệu quả, an toàn và phát triển bền vững cho công nghiệp toàn cầu.",
  "Company": "Công ty",
  "Offering": "Sản phẩm & dịch vụ",
  "Get in touch": "Liên hệ",
  "Product finder": "Tra cứu sản phẩm",
  "All rights reserved.": "Bảo lưu mọi quyền.",
  "Manufacturer & exporter of speciality chemicals":
    "Nhà sản xuất & xuất khẩu hóa chất đặc chủng",
};

export default vi;
